"""
冻结版微调: 冻结CLIP Encoders，只训练投影层和ArcFace头

与原版区别：
- CLIP vision_model 和 text_model 参数冻结
- 保持CLIP原始特征提取能力
- 只学习如何更好地映射和融合特征
- 训练更快，参数更少
"""
...
# ============ 冻结版多模态模型 ============
class FrozenMultiModalModel(nn.Module):
    """
    冻结版：CLIP encoders冻结，只训练投影层和ArcFace头
    """

    def __init__(self, clip_dir: str, num_classes: int, embed_dim: int = 512, freeze_clip: bool = True):
        super().__init__()
        ...
        self.fusion = nn.Sequential(
            nn.Linear(clip_dim * 2, embed_dim * 2),
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/enhanced_frozen")
    parser.add_argument("--val_ratio", type=float, default=0.2)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=2e-4)  # 可以用更大学习率，因为参数少
    parser.add_argument("--topk", type=int, default=50)
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")

    # 损失权重
    parser.add_argument("--w_img", type=float, default=0.3)
    parser.add_argument("--w_txt", type=float, default=0.3)
    parser.add_argument("--w_fused", type=float, default=0.4)

    # 评估时的融合权重
    parser.add_argument("--eval_alpha", type=float, default=0.4)

    # 是否冻结CLIP（默认冻结）
    parser.add_argument("--no_freeze", action="store_true", help="不冻结CLIP encoders")
    ...
    freeze_clip = not args.no_freeze
    model = FrozenMultiModalModel(
        args.clip_dir, num_classes, embed_dim=512, freeze_clip=freeze_clip
    ).to(args.device)
